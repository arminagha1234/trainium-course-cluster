# Trainium Course Cluster — AWS PCS variant

A parallel implementation of the Trainium Course Cluster kit that runs the
Slurm cluster on **AWS Parallel Computing Service (PCS)** instead of AWS
ParallelCluster. Same goal — a shared Trainium Slurm cluster for NKI kernel
assignments, backed by a pre-purchased ML Capacity Block (CB) — different
control plane: PCS provides a managed Slurm controller, so this kit hands PCS a
launch template + IAM + networking and PCS owns the scheduler lifecycle.

This lives alongside the ParallelCluster kit (`../`) and does **not** modify it.
It reuses that kit's Lambdas and harness, and ships its own PCS-specific Slurm
job template (see [Reuse from the parent kit](#reuse-from-the-parent-kit)).
...
- **Reduced scheduling granularity vs the ParallelCluster kit.** `neuroncore`
  can't be a Slurm GRES on PCS — it is in neither the CNG nor the cluster
  custom-settings allow-list — so there is **no per-core scheduling** and **no
  per-core / core-hours budget**. This is now **confirmed on a live PCS trn2
  node**: `scontrol show node trn2cb-1` reports **`Gres=(null)`** (the managed
  node advertises no gres, with no autodetect fallback), so it is not merely
  inferred from the docs. Nodes are selected whole with `--constraint=neuron`
  (use `--exclusive` for isolation), and per-student limits are wall-time +
  concurrent-job-count only. If per-NeuronCore sharing matters for your course,
  prefer the ParallelCluster kit (`../`). See
  [`docs/design.md`](./docs/design.md) "PHASE STATUS" (Phase 2).
- Treat this as **"works but officially unsupported."** Validate again on your
  own account/region before relying on it for a live class, and keep the
  ParallelCluster kit (`../`) as the supported fallback.
...
This variant only re-implements the cluster control plane. The following pieces
from the ParallelCluster kit (`../`) are **reused as-is** and are intentionally
not duplicated here:
...
The **Slurm job templates are NOT reused as-is.** The parent kit's
`../slurm/job-templates/*.sh` request NeuronCores with `--gres=neuroncore:N`,
which **does not schedule on PCS** — `neuroncore` can't be a Slurm GRES here
(see the caveat above and `docs/design.md` "PHASE STATUS" Phase 2). The PCS
variant therefore ships its own `slurm/job-templates/run.sh` that selects nodes
with `--constraint=neuron`; it still sources the shared
`/opt/aws_neuronx_venv_pytorch` venv that `neuron-userdata.sh` builds.
...
PCS deploy — all **implemented but not yet live-validated**. Still open (reuse,
not yet wired): the `student_manifest` Lambda as the roster source, and the
...
- Parent kit (supported ParallelCluster path): [`../README.md`](../README.md)
